...
func (cm *ConsensusModule) processSendAEsReply(peerId int, reply AppendEntriesReply, savedCurrentTerm int, ni int, args AppendEntriesArgs) {
	cm.mu.Lock()
	defer cm.mu.Unlock()
	if reply.Term > savedCurrentTerm {
